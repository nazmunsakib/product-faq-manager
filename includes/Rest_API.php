...
class Rest_API {
     ...
     * - /product-faq-manager/v1/set-faq: To add or remove FAQs for a product.
     * - /product-faq-manager/v1/get-faq: To retrieve FAQs associated with a product.
     */
    public function custom_api_route() {
        // Route for adding/removing an FAQ to/from a product.
        register_rest_route('product-faq-manager/v1', '/set-faq', array(
            'methods'   => 'POST',
            ...
            'permission_callback' => function(WP_REST_Request $request) {
                ...
                if (!wp_verify_nonce($nonce, 'wp_rest')) {
                    return new WP_Error('rest_forbidden', esc_html__('Invalid nonce', 'product-faq-manager'), ['status' => 403]);
                }

                // Check if user is logged in.
                if (!is_user_logged_in()) {
                    return new WP_Error('rest_forbidden', esc_html__('You do not have permission to access this resource.', 'product-faq-manager'), ['status' => 403]);
                }
        ...
        register_rest_route('product-faq-manager/v1', '/get-faq', array(
            'methods'   => 'POST',
    ...
    public function set_faq_callback(WP_REST_Request $request) {
        ...
        $faqs_ids = get_post_meta($product_id, 'pfaqm_faq_product_ids', true);
        ...
        switch($action) {
            case "add":
                if (!in_array($faq_id, $faqs_ids)) {
                    ...
                    update_post_meta($product_id, 'pfaqm_faq_product_ids', $faqs_ids);
                }
